...
async function fetchHexJsonWithCache() {
    ...
    if (res.status === 304) {
        ...
        const forceRes = await fetch('/api/hexjson', {
            cache: 'no-store'
        });

        if (!forceRes.ok) {
            throw new Error(`HTTP ${forceRes.status}`);
        }
...
function renderChartsWithData(data) {
    ...
    if (tshareEl) {
        tshareEl.textContent = latest.tshareRateHEX
            ? `${formatWithCommas(latest.tshareRateHEX.toFixed(0))} HEX`
            : '0 HEX';
    }
    ...
    if (payoutEl) {
        payoutEl.textContent = latest.payoutPerTshareHEX
            ? `${formatWithCommas(latest.payoutPerTshareHEX.toFixed(2))} HEX`
            : '0.000 HEX';
    }
    ...
    if (dailyEl) {
        dailyEl.textContent = latest.dailyPayoutHEX
            ? `${formatWithCommas(latest.dailyPayoutHEX.toFixed(0))} HEX`
            : '0 HEX';
    }
...
function renderPortfolioHistoryChartWithData(rawData) {
    ...
    const portfolio = sorted.map(e => ({
        day: e.currentDay,
        value:
            userTotalTShares * (e.tshareRateHEX * e.pricePulseX) +
            userLiquidHEX * e.pricePulseX
    }));
...
async function fetchLiveDataAndRender() {
    ...
    try {
        ...
        if (!isValidLiveData(data)) {
            throw new Error('Live data invalid or zero price');
        }
...
function updateLiveDataUI(data) {
    ...
    document.getElementById('tshare-rate').textContent =
        `${formatWithCommas(Math.floor(tshareRate.toFixed(0)))} HEX`;
    document.getElementById('payout').textContent = `${payout.toFixed(2)} HEX`;
    document.getElementById('penalties').textContent =
        `${formatWithCommas(Math.floor(penalties.toFixed(0)))} HEX`;
    document.getElementById('beat').textContent =
        `${formatWithCommas(Math.floor(beat.toFixed(0)))}`;
...
function setupLiveDataInterval(freq) {
    ...
    // Invalidate any in-flight fetch scheduling.
    liveFetchGeneration++;
        ...
        .then(miners => {
            ...
            if (!completed.length) {
                ...
                completed.forEach(m => {
                    ...
                    li.textContent =
                        `${m.startDate} - ${m.endDate} • T-Shares: ${tShares.toFixed(2)}`;
...
async function endMiner(id) {
    ...
    if (!Number.isFinite(minerId)) {
        showNotification('Invalid miner ID', 'danger');
        return;
    }
    ...
    try {
        const res = await fetch('/api/end-miner', {
            ...
            body: JSON.stringify({ id: minerId })
        });
...
async function deleteMiner(id) {
    ...
    try {
        ...
        showNotification(
            res.ok ? 'Miner deleted' : 'Error deleting miner',
            res.ok ? 'success' : 'danger'
        );
...
async function initialLoad() {
    try {
        ...
        document.getElementById('profile-message').textContent =
            activeMiners.length ? '' : 'Empty profile. Please add HEX miners in Settings.';
        ...
        activeMiners.forEach(m => {
            ...
            header.className = 'miner-card-header';

            const dates = document.createElement('div');
            dates.className = 'miner-dates';

            const startBlock = document.createElement('div');
            const startLabel = document.createElement('span');
            startLabel.className = 'miner-date-label';
            startLabel.textContent = 'Start Date';

            const startValue = document.createElement('div');
            startValue.className = 'miner-date-value';
            startValue.textContent = m.startDate;

            startBlock.appendChild(startLabel);
            startBlock.appendChild(startValue);

            const endBlock = document.createElement('div');
            const endLabel = document.createElement('span');
            endLabel.className = 'miner-date-label';
            endLabel.textContent = 'End Date';

            const endValue = document.createElement('div');
            endValue.className = 'miner-date-value';
            endValue.textContent = m.endDate;

            endBlock.appendChild(endLabel);
            endBlock.appendChild(endValue);

            dates.appendChild(startBlock);
            dates.appendChild(endBlock);

            const tshares = document.createElement('div');
            tshares.className = 'miner-tshares';

            const tsharesLabel = document.createElement('div');
            tsharesLabel.className = 'miner-tshares-label';
            tsharesLabel.textContent = 'T-Shares';

            const tsharesValue = document.createElement('div');
            tsharesValue.className = 'miner-tshares-value';
            tsharesValue.textContent = (Number(m.tShares) || 0).toFixed(2);

            tshares.appendChild(tsharesLabel);
            tshares.appendChild(tsharesValue);

            header.appendChild(dates);
            header.appendChild(tshares);

            const progressContainer = document.createElement('div');
            progressContainer.className = 'miner-progress-container';

            const progressInfo = document.createElement('div');
            progressInfo.className = 'miner-progress-info';

            const daysLeftSpan = document.createElement('span');
            daysLeftSpan.className = 'miner-days-left';

            if (matured) {
                const strong = document.createElement('strong');
                strong.textContent = 'Matured';
                daysLeftSpan.appendChild(strong);
            } else {
                const strong = document.createElement('strong');
                strong.textContent = String(daysLeft);
                daysLeftSpan.appendChild(strong);
                daysLeftSpan.appendChild(document.createTextNode(' days remaining'));
            }

            const percentageSpan = document.createElement('span');
            percentageSpan.className = 'miner-percentage';
            percentageSpan.textContent = `${percentage.toFixed(1)}%`;

            progressInfo.appendChild(daysLeftSpan);
            progressInfo.appendChild(percentageSpan);

            const progressBar = document.createElement('div');
            progressBar.className = 'miner-progress-bar';
            ...
            footer.className = 'miner-card-footer';

            if (matured) {
                const endBtn = document.createElement('button');
                endBtn.className = 'btn btn-sm btn-danger';
                endBtn.textContent = 'End Miner';
                ...
                badge.className = 'miner-status-badge active';
        ...
        miners.forEach(m => {
            ...
            const span = document.createElement('span');
            span.textContent =
                `${m.startDate} - ${m.endDate} • T-Shares: ${(Number(m.tShares) || 0).toFixed(2)}`;
