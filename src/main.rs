...
mod tests {
    ...
    fn test_parse_date_valid() {
        ...
        assert_eq!(parse_date("29-02-2024"), Some((2024, 2, 29)));
    }
    ...
    async fn test_handle_add_miner_success() {
        ...
        assert_eq!(miners[0].id, Some(1));
        assert_eq!(miners[0].status, None);
    }
    ...
    #[tokio::test]
    async fn test_handle_delete_miner_not_found() {
        let state = create_test_state();

        let req = MinerIdRequest { id: 99 };
        let response = handle_delete_miner(State(state.clone()), Json(req)).await;
    ...
    async fn test_config_sanitization() {
        let config = Config {
            live_data_frequency: 0,
            liquid_hex: -10.0,
            historical_start_day: 0,
        };

        let sanitized = sanitize_config(config);

        assert_eq!(sanitized.live_data_frequency, 1);
        assert_eq!(sanitized.liquid_hex, 0.0);
        assert_eq!(sanitized.historical_start_day, 1);
    }
}
