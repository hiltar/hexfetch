...
async fn main() {
    ...
    let initial_config = match tokio::fs::read_to_string(&config_path).await {
    ...
    let miners_path = data_dir.join("miners.json");
    let initial_miners = match tokio::fs::read_to_string(&miners_path).await {
